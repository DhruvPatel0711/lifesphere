...
const AppDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('impersonate') === '1') {
      ...
      if (impToken) {
        localStorage.setItem('lifesphere_token', impToken);
        if (impRefresh) localStorage.setItem('lifesphere_refresh_token', impRefresh);
  ...
  const currentPath = location.pathname.length > 5 && location.pathname.endsWith('/')
    ? location.pathname.slice(0, -1)
    : location.pathname;

  if (!validAppPaths.includes(currentPath)) {
    return <NotFound />;
  }
  ...
  const careItems = [
    { id: 'family', label: 'Family Hub', path: '/app/family', icon: Users },
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 flex bg-gray-50 dark:bg-black text-gray-900 dark:text-gray-100 font-sans overflow-hidden">
      ...
      {/* Global AI Chatbot & Universal Voice Assistant */}
      <LandingChatbot />
