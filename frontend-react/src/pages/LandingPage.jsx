        ...
        <nav className="flex justify-between items-center px-6 md:px-10 py-4 max-w-[1440px] mx-auto">
          <div className="flex flex-1 items-center gap-3 cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
            <div className="w-9 h-9 rounded-[10px] bg-[#0ea5e9] flex items-center justify-center text-white shadow-sm">
              {settings?.site_logo ? (
                <img src={settings.site_logo} alt="LifeSphere Platform Logo" className={`w-full h-full object-contain ${isDark ? 'bg-transparent' : 'bg-white'}`} />
              ) : (
                <HeartPulse size={20} strokeWidth={2.5} />
              )}
            </div>
            <div className={`text-[22px] font-extrabold tracking-tight ${isDark ? 'text-white' : 'text-[#0f172a]'}`}>LifeSphere</div>
          </div>
          {/* Desktop Navigation */}
              ...
              try {
                ...
                return (
                  <a href="/medical-id" className={`flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-full text-xs font-bold transition-colors no-underline ${isDark ? 'bg-red-500/10 hover:bg-red-500/20 text-red-400' : 'bg-red-50 text-red-600 hover:bg-red-100'}`}>
                    {avatar ? (
                      <img src={avatar.startsWith('http') ? avatar : `http://localhost:8000${avatar}`} alt="User Profile Avatar" className="w-[18px] h-[18px] rounded-full object-cover" />
                    ) : (
      ...
      <main className={`pt-[74px] overflow-hidden transition-colors duration-300 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        ...
        <section id="stats" className="py-24 px-6 md:px-10 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
          <div className="max-w-7xl mx-auto text-center">
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-12 reveal-on-scroll opacity-0 translate-y-12 transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)]">Why choose LifeSphere</h2>
        ...
        {/* ========== 5 ACCORDION FAQs (Item 7) ========== */}
        <section id="faq" className="py-20 px-6 md:px-10 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              <div className="text-blue-600 dark:text-blue-400 font-bold tracking-widest text-sm mb-2 uppercase">Got Questions?</div>
              <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white">Frequently Asked Questions</h2>
            </div>
            <div className="space-y-4">
              {[
                { q: "Is LifeSphere a substitute for a real doctor?", a: "No. LifeSphere provides preliminary triage, symptom analysis, and health tracking. Always consult a licensed medical professional for formal clinical diagnosis." },
                { q: "How is my medical data protected?", a: "Your health records are stored with end-to-end encryption. We enforce a zero data-selling policy and never monetize your personal diagnostic data." },
                { q: "Can I manage health records for my family?", a: "Yes! Our Family Hub lets you manage separate health profiles, active medications, and vaccination schedules for your parents, spouse, or children." },
                { q: "How fast does the AI process lab report PDFs?", a: "Lab report text and health metrics (like Hemoglobin, Blood Sugar, Cholesterol) are extracted and summarized in sub-2 seconds." },
                { q: "What should I do in an acute emergency?", a: "Use the Emergency SOS tab in your dashboard for 1-tap emergency contacts dispatch, nearby hospital navigation, and digital Medical ID generation." }
              ].map((faq, i) => (
                <details key={i} className="group bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 transition-all cursor-pointer">
                  <summary className="font-bold text-slate-900 dark:text-white flex items-center justify-between text-base">
                    <span>{faq.q}</span>
                    <span className="text-blue-500 font-bold text-lg group-open:rotate-180 transition-transform">+</span>
                  </summary>
        ...
        {/* ========== MEDICAL ADVISORY & TEAM SECTION (Item 20) ========== */}
        <section id="team" className="py-20 px-6 md:px-10 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800">
          <div className="max-w-6xl mx-auto text-center">
            <div className="text-blue-600 dark:text-blue-400 font-bold tracking-widest text-sm mb-2 uppercase">Our Team</div>
            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white mb-12">Engineered by Medical AI Engineers & Physicians</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm text-center">
                <div className="w-20 h-20 bg-blue-500/10 text-blue-500 rounded-full flex items-center justify-center text-3xl mx-auto mb-4 font-bold">👨‍💻</div>
                <h3 className="font-bold text-slate-900 dark:text-white text-lg">Gaurav Patel</h3>
                <p className="text-xs text-blue-500 font-semibold mb-2">Lead Systems Architect</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Specializes in FastAPI microservices, JWT security & database architecture.</p>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm text-center">
                <div className="w-20 h-20 bg-purple-500/10 text-purple-500 rounded-full flex items-center justify-center text-3xl mx-auto mb-4 font-bold">🧠</div>
                <div className="font-bold text-slate-900 dark:text-white text-lg">Dhruv Patel</div>
                <p className="text-xs text-purple-500 font-semibold mb-2">AI Medical Engineer</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Specializes in Groq Llama 3.3 model prompts, RAG & clinical lab report OCR.</p>
              </div>
              <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm text-center">
                <div className="w-20 h-20 bg-emerald-500/10 text-emerald-500 rounded-full flex items-center justify-center text-3xl mx-auto mb-4 font-bold">🎨</div>
                <div className="font-bold text-slate-900 dark:text-white text-lg">Ridhham Solanki</div>
                <p className="text-xs text-emerald-500 font-semibold mb-2">UI/UX & Product Design</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">Designed fluid responsive glassmorphism health dashboards and mobile interfaces.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center text-white overflow-hidden">
              {settings?.site_logo ? (
                <img src={settings.site_logo} alt="LifeSphere Platform Logo" className="w-full h-full object-contain bg-white dark:bg-transparent" />
              ) : (
                <span className="material-symbols-outlined text-[20px]" style={{ fontVariationSettings: "'FILL' 1" }}>favorite</span>
              )}
            </div>
            <div className="text-xl font-bold text-white tracking-tight">LifeSphere</div>
          </div>
          <div className="flex flex-wrap justify-center gap-6 text-sm font-medium">
            <a className="text-slate-300 hover:text-white transition-colors" href="/app">Dashboard</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/app/ai-symptom">AI Triage</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/app/trackers">Smart Trackers</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/app/emergency">Emergency SOS</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/app/medicine">Medications</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/privacy">Privacy Policy</a>
            <a className="text-slate-300 hover:text-white transition-colors" href="/privacy">Terms of Service</a>
          </div>
          <div className="text-sm text-slate-500">
            &copy; {new Date().getFullYear()} LifeSphere. All rights reserved.
          </div>
        </div>
      </footer>

      {/* ========== STICKY MOBILE CTA (Item 9) ========== */}
      <div className="fixed bottom-4 left-4 right-4 z-40 md:hidden bg-slate-900/90 dark:bg-slate-950/90 backdrop-blur-md border border-slate-700/50 rounded-2xl p-3 shadow-2xl flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-xs font-bold text-white">LifeSphere Ready</span>
        </div>
        <button
          onClick={() => setShowLogin(true)}
