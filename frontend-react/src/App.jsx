...
const AppRoutes = () => {
  ...
  useEffect(() => {
    // Dynamic Page Titles (Item 11) & Meta Descriptions (Item 12)
    const titleMap = {
      '/': 'LifeSphere — AI-Powered Healthcare Operating System',
      '/privacy': 'Privacy Policy — LifeSphere',
      '/medical-id': 'Emergency Medical ID — LifeSphere',
      '/admin/login': 'Admin Portal Login — LifeSphere',
    };

    const descMap = {
      '/': 'Next-generation AI healthcare platform offering AI symptom checking, medical record vault, family health management, nutrition planning, and 24/7 emergency dispatch.',
      '/privacy': 'Read our strict privacy policy and security standards for protecting your sensitive medical data.',
      '/medical-id': 'Your Emergency Medical ID. Quick access to critical health information for first responders.',
      '/admin/login': 'Secure administrative portal for LifeSphere system management.',
    };

    let newTitle = titleMap[location.pathname] || 'LifeSphere';
    let newDesc = descMap[location.pathname] || 'AI-powered healthcare operating system for medical record analysis, family health tracking, and symptom checking.';

    if (location.pathname.startsWith('/app/family')) {
      newTitle = 'Family Hub | LifeSphere';
      newDesc = 'Manage health profiles, medications, and vaccination schedules for your entire family.';
    } else if (location.pathname.startsWith('/app/records')) {
      newTitle = 'Medical Records | LifeSphere';
      newDesc = 'Securely store and analyze your lab reports, prescriptions, and medical records with AI.';
    } else if (location.pathname.startsWith('/app/appointments')) {
      newTitle = 'Appointments | LifeSphere';
      newDesc = 'Schedule and manage your upcoming doctor appointments and medical visits.';
    } else if (location.pathname.startsWith('/app/medicine')) {
      newTitle = 'Medication Manager | LifeSphere';
      newDesc = 'Track your active prescriptions, set reminders, and manage your medication schedule.';
    } else if (location.pathname.startsWith('/app/emergency')) {
      newTitle = 'Emergency SOS | LifeSphere';
      newDesc = '1-tap emergency dispatch, hospital navigation, and critical medical profile sharing.';
    } else if (location.pathname.startsWith('/app')) {
      newTitle = 'Patient Dashboard | LifeSphere';
      newDesc = 'Your intelligent personal healthcare dashboard and AI medical assistant.';
    } else if (location.pathname.startsWith('/admin')) {
      newTitle = 'System Administration | LifeSphere';
      newDesc = 'LifeSphere administrative dashboard and system management.';
    }
  ...
  useEffect(() => {
    if (isMaintenance && !isAdminRoute) {
      ...
      if (token && !localStorage.getItem('admin_logged_in')) {
        ...
        localStorage.removeItem('lifesphere_accounts');
