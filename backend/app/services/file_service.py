"""
LifeSphere Backend — File Upload Service
...
logger = logging.getLogger("lifesphere.files")
...
def get_file_path(relative_path: str) -> Path | None:
    """Get the absolute path of an uploaded file, preventing path traversal."""
    settings = get_settings()
    base_dir = Path(settings.UPLOAD_DIR).resolve()
    # Remove leading slashes to prevent absolute path injection before joining
    safe_rel_path = str(relative_path).lstrip("/\\")
    full_path = (base_dir / safe_rel_path).resolve()
...
def delete_file(relative_path: str) -> bool:
    """Delete an uploaded file, preventing path traversal."""
