"""
LifeSphere Backend — Authentication Router
...
logger = logging.getLogger("lifesphere.auth")
...
async def login(data: LoginRequest, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    response.set_cookie(key="lifesphere_access_token", value=access_token, httponly=True, samesite="lax", max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60)
    response.set_cookie(key="lifesphere_refresh_token", value=refresh_token, httponly=True, samesite="lax", max_age=60 * 60 * 24 * 7)
...
async def verify_email(data: VerifyEmailRequest, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    # Check expiration if normal token record
    if token_record and datetime.now(timezone.utc) > token_record.expires_at.replace(tzinfo=timezone.utc):
        raise UnauthorizedException("Verification code has expired")
...
async def refresh_token(request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    refresh_token = request.cookies.get("lifesphere_refresh_token")
    ...
    response.set_cookie(key="lifesphere_access_token", value=access_token, httponly=True, samesite="lax", max_age=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60)
    response.set_cookie(key="lifesphere_refresh_token", value=new_refresh, httponly=True, samesite="lax", max_age=60 * 60 * 24 * 7)
...
async def logout(response: Response, request: Request):
    ...
    user_id = "unknown"
    try:
        token = request.cookies.get("lifesphere_access_token")
        if token:
    ...
    response.delete_cookie("lifesphere_access_token")
    response.delete_cookie("lifesphere_refresh_token")
...
async def setup_2fa(user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    ...
    uri = totp.provisioning_uri(name=user.email, issuer_name="LifeSphere")
