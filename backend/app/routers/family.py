"""
LifeSphere Backend — Family & Vaccination Router
"""
...
async def add_vaccination(data: VaccinationCreate, user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    ...
    if data.family_member_id:
        member_res = await db.execute(
            select(FamilyMember).where(FamilyMember.id == data.family_member_id, FamilyMember.user_id == user_id)
        )
        if not member_res.scalar_one_or_none():
            raise NotFoundException("Family member", data.family_member_id)
...
async def update_vaccination(
    vax_id: str, data: VaccinationUpdate, user_id: CurrentUserId, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(Vaccination).where(Vaccination.id == vax_id, Vaccination.user_id == user_id)
    )
    vax = result.scalar_one_or_none()
    if not vax:
        raise NotFoundException("Vaccination record", vax_id)
...
async def delete_vaccination(vax_id: str, user_id: CurrentUserId, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Vaccination).where(Vaccination.id == vax_id, Vaccination.user_id == user_id)
    )
    vax = result.scalar_one_or_none()
    if not vax:
        raise NotFoundException("Vaccination record", vax_id)
    await db.delete(vax)
    return {"success": True, "message": "Vaccination record removed"}
