...
async def test_all_ai():
    ...
    out.append("==================================================")
    out.append("[TEST] DEEP TESTING ALL AI FEATURES IN HEALTHCARE AI")
    out.append(f"Groq API Key Configured: {bool(settings.GROQ_API_KEY)}")
    out.append(f"OpenRouter API Key Configured: {bool(settings.OPENROUTER_API_KEY)}")
    out.append(f"Groq Model: {settings.GROQ_MODEL}")
    out.append("==================================================")
    ...
    try:
        ...
        out.append(f"AI Assistant Response (first 150 chars):\n{res[:150]}...")
        ...
        out.append(f"[FAIL] Core Assistant Error: {e}")
    ...
    try:
        ...
        out.append(f"Symptom JSON Response Keys: {list(json_res.keys())}")
        out.append(f"Full Symptom Output: {json.dumps(json_res, indent=2)[:300]}...")
        ...
        out.append(f"[FAIL] Symptom Triage Error: {e}")
    ...
    if os.path.exists(pdf_path):
        try:
            ...
            doc.close()
            out.append(f"PDF Extracted Text Length: {len(extracted_text)} chars")
            out.append(f"Sample PDF Text Snippet: {extracted_text[:200].strip()}...")
            ...
            out.append(f"Parsed Metrics from PDF: {json.dumps(parsed_metrics, indent=2)}")
            ...
            out.append(f"[FAIL] PDF Parser Error: {e}")
            results["pdf_parsing"] = f"FAIL: {e}"
    else:
        out.append(f"[WARN] PDF file not found at {pdf_path}")
    ...
    try:
        ...
        out.append(f"AI Summary Result:\n{summary_res[:200]}...")
        ...
        out.append(f"[FAIL] Record Summary Error: {e}")
    ...
    try:
        ...
        out.append(f"AI Comparison Result:\n{compare_res[:200]}...")
        results["record_comparison"] = "PASS"
    except Exception as e:
        out.append(f"[FAIL] Record Comparison Error: {e}")
    ...
    try:
        ...
        out.append(f"AI Nutrition Result:\n{nutrition_res[:200]}...")
        results["nutrition_planner"] = "PASS"
    except Exception as e:
        out.append(f"[FAIL] Nutrition Error: {e}")
    ...
    try:
        ...
        out.append(f"AI Fitness Result:\n{fitness_res[:200]}...")
        results["fitness_coach"] = "PASS"
    except Exception as e:
        out.append(f"[FAIL] Fitness Error: {e}")
    ...
    try:
        ...
        out.append(f"AI Mental Result:\n{mental_res[:200]}...")
        results["mental_support"] = "PASS"
    except Exception as e:
        out.append(f"[FAIL] Mental Error: {e}")
        results["mental_support"] = f"FAIL: {e}"

    out.append("\n==================================================")
    out.append("[SUMMARY] COMPREHENSIVE AI FEATURE TEST SUMMARY")
    out.append("==================================================")
    for feature, status in results.items():
        out.append(f"* {feature.upper()}: {status}")

    with open("test_ai_output.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(out))
    print("Wrote test results to test_ai_output.txt")
